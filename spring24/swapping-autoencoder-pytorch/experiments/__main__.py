...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_structure_image', type=str, help='Path to input structure image')
    parser.add_argument('--input_texture_image', type=str, help='Path to input texture image')
    ...
    if cmd == "launch":
        ...
        if "all" in ids and len(ids) == 1:
            test_commands = instance.test_commands(opt.input_structure_image, opt.input_texture_image)
            ids = list(range(len(test_commands)))
        else:
            test_commands = [instance.test_commands(opt.input_structure_image, opt.input_texture_image)[instance.find_tag(instance.test_options(opt.input_structure_image, opt.input_texture_image), tag)] for tag in ids]
        for expid, test_command in enumerate(test_commands):
            instance.run_command([test_command], None, opt.resume_iter,
                                 gpu_id=opt.gpu_id)
            if expid < len(test_commands) - 1:
                os.system("sleep 5s")
    elif cmd == "plot_loss":
