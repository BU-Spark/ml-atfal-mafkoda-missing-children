...
- **Purpose**: Implements face detection on a set of images using the `DeepFace` library, capable of handling images where no faces are detected.
- **Features**:
  ...
  - **Handling Undetected Faces**: Provides options to either remove or relocate images with undetected faces.
  - **Output Generation**: Generates a CSV file listing images in which faces were not detected.
- **Configuration Flexibility**: Accommodates both hardcoded and command-line arguments for diverse detection scenarios.
...
The `inputs_outputs` directory contains essential files used in and generated by the project's notebooks. This includes a JSON file with Facebook posts data used as input for `data_downloader.ipynb`, the `jsontocsv.csv` file generated in `data_explore.ipynb`, and two output CSV files, `matches_cosine.csv` and `matches_euclideanl2.csv`, produced using various metrics in `face_matching.ipynb`.
...
- **Face Detection**: Images are initially processed through `face_detector.ipynb`, employing the MTCNN algorithm for face detection.
- **Face Matching**: Detected faces are matched using `face_matching.ipynb`. This notebook uses DeepFace with 'VGG-Face' as the model for recognition and 'mtcnn' as the detector backend. Matching is performed using two metrics: Euclidean L2 and Cosine.
...
- This part of the analysis evaluates the precision of matches by examining score ranges and the position of the first match in the top results, crucial for assessing the accuracy of early matches.
